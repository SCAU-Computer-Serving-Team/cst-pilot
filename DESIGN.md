...
登录页以 1080×1080、圆角 32 为设计基准，正式页面按 90% 等比呈现为 972×972、圆角 28.8 的居中面板。面板边长不超过视口高度的 90%，左右至少留 24px；空间不足时继续等比缩小。

以下尺寸及登录投影参数均按设计基准标注。面板内的字号、控件、间距、圆角、描边与投影随面板同步缩放；AIR 背景铺满面板，背后真实主页保持原尺寸并模糊。
...
| APIKEY 表单 · 浅色 | 复用 480×480 登录卡片；输入框高 42、圆角 8、白底、描边 `#D5D5D5`；Provider 与 APIKEY 字段保留可见标签 |
| 自定义 Provider | 勾选后在 APIKEY 上方增加 BaseURL；Provider 与登录按钮的位置保持不变；取消勾选隐藏 BaseURL |
| Provider 下拉 | 独立浅深组件画布；宽 424，与输入框同宽，向下间隔 8；复用模型选择面板的圆角、描边、投影与选中行样式；选项文字与输入框文字左对齐，选中勾位于行末 |
| APIKEY 表单 · 深色 | 卡片 `#131A1D`，输入底 `#0B1215`、描边 `#3C4448`；主按钮 `#7785DE` 配 `#001F42` 文字 |
...
各页取值、尺寸与形态直接看 [界面画布](src/web/design/cst-pilot-web.pen)，本文只记画布上看不出来的规则。登录页已有扫码态浅深两版，以及 APIKEY 默认态、自定义 Provider 态和 Provider 下拉的浅深画布。
