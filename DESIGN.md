...
以下是现有品牌素材。Web 标识的用法与尺寸列入[资源待办](doc/web/issues.md#3-视觉与资源)。
...
字体按界面角色指定，不按每个字符自动切换。例如账户名 `Tim2354` 在当前画布中仍使用思源黑体。网页字体分发、加载与缺字回退方案见[资源待办](doc/web/issues.md#3-视觉与资源)。
