...
登录页以 1080×1080、圆角 32 为设计基准，正式页面按 80% 等比呈现为 864×864、圆角 25.6 的居中面板。面板边长不超过视口高度的 80%，左右至少留 24px；空间不足时继续等比缩小。
...
| 登录页 | 「登录页 · 浅色」「登录页 · 深色」，以及两个「呈现效果」帧 | 正式面板按设计基准的 80% 等比呈现，标准尺寸 864×864，与两个「呈现效果」帧一致；窗口适配、配色、描边与投影见[登录页面板](#登录页面板)。正式实现模糊背后的真实主页，面板保持清晰 |
