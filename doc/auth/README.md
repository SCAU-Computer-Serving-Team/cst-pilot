...
7. 登录入口分两种：TUI 按 pi 原生界面显示网址与数字码；Web UI 显示二维码（编码 verification_uri_complete）。两者走同一套设备流与批准逻辑。
...
| 登录界面 | TUI：pi 原生设备码界面（网址 + 6 位数字码）；Web UI：登录面板渲染二维码（编码 verification_uri_complete） |
...
| 二维码 | Web UI 渲染（扩展提供数据）；TUI 暂不做，上游 issue #9774 仅涉及 TUI |
...
| D12 | 登录界面与二维码 | TUI：pi 原生链接 + 数字码；Web UI：渲染二维码；上游 issue #9774 仅涉及 TUI |
