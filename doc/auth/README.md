...
OAuth 只负责队员登录认证和接入团队提供的模型服务，不与修机任务绑定。登录和模型调用均不要求选择任务、创建修机会话或提供任务编号。
...
flowchart LR
    subgraph PC["机主电脑（不可信环境）"]
        ...
        CRED["U 盘凭据 auth.json<br/>设备标识 device.json"]
        AG --- CRED
    end
...
| ⑤ | agent → 后端 | 可选的任务与日志业务，不影响登录和模型调用 | 独立业务接口 |
...
1. 模型接入改为 OA 签发的短期凭据，网关的长期 API key 留在服务端。
2. 主通道是 OAuth 2.0 设备授权流（RFC 8628），队员在手机打开授权入口、输入数字码并确认。
3. 授权主体是队员。机主不是 OA 用户，只出现在独立的修机业务中。
4. 登录态按令牌有效期延续，不因换电脑或更换修机任务要求重新授权；「记住 7 天」从首次授权起算，不随刷新延长。
5. OAuth 不与修机任务绑定。登录成功且权限、额度满足要求后即可调用模型。
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| M1 | 设备授权流、TOTP、设备管理、LLM 代理端点、pi 扩展 | 本 spec 主体 |
| M2 | 修机日志、遥测上报、附件 | 概要 |
| M3 | 任务生命周期动作、委员只读 | 概要 |
...
| 网关 | New API 大模型网关 |
| 修机会话 | 独立修机业务的记录，不代表 OAuth 登录态，也不是模型调用的前提 |
| 任务 | OA 任务中心的一条修机工单 |
...
| 8 | agent | 正常对话与诊断（模型经 OA 代理，无需选择任务） |
| 9 | OA | 到期或吊销后失效 |
...
3. 授权页显示设备与作用域，不绑定修机任务。
4. 默认不记住时，访问令牌临期后按 pi 的刷新流程提示重新授权；开启「记住 7 天」后可自动续期。
5. 同一 U 盘跨电脑或盘符变化时，凭据继续由 U 盘上的 `auth.json` 提供，不因换电脑额外要求登录。
...
1. OAuth 只确认队员身份并授予模型服务访问权限，不负责修机任务或修机会话的生命周期。
2. 模型请求不要求携带任务编号或修机会话编号。没有任务、未选任务或没有创建修机会话时，不因此拒绝模型调用。
3. 任务选择、日志提交及其关联关系由独立业务规格定义，不纳入 OAuth 的 M1 验收。
4. 独立业务可以按作用域复用当前登录身份，但不能反过来成为登录或模型调用的前提。
...
| ANY | /api/agent/llm/v1/* | access_token | OpenAI 兼容代理，转发 New API（流式），不要求任务编号 |

独立业务接口预留，不属于 OAuth M1，也不是模型调用的前提：

| 方法 | 路径 | 鉴权 | 说明 |
|---|---|---|---|
| GET | /api/agent/tasks | access_token | 本人任务列表 |
| POST | /api/agent/repair-sessions | access_token | 创建修机业务记录 |
...
| 命令 | /login 走 pi 原生流程；任务和日志命令由独立业务扩展提供 |
...
| 审计字段 | 队员、设备、作用域、接口、结果、IP、时间；任务与修机会话信息仅用于独立业务审计 |
| 审计保留 | 180 天 |
...
4. 同一 U 盘换到另一台电脑、盘符变化：登录态按有效期延续，不要求选择任务或创建修机会话。
5. 登录成功且权限、额度满足要求后，未选择任务、未创建修机会话也能调用模型。
...
| 阶段 | 交付 |
|---|---|
| M1 | OA 设备授权流、TOTP、设备管理页、LLM 代理端点；pi 扩展；模型调用 |
| M2 | 修机日志草稿与提交；遥测合并上报 |
...
3. OA 侧的任务拆分见 cstoa-api-fastapi 仓库 [docs/oa-tasks.md](https://github.com/SCAU-Computer-Serving-Team/cstoa-api-fastapi/blob/main/docs/oa-tasks.md)。认证与模型接入边界以本文为准，OA 侧拆分需同步，不以任务绑定作为 OAuth 开发前提。
