...
采什么、不采什么由[信息收集契约](../contract.md)决定，不由队员选。
...
| L4 | 可关闭；关闭后不采集、不落盘、不联网 | sender「配置」的 `enabled`；「响应与处理」功能关闭一行 |
...
| [../contract.md](../contract.md) | 信息收集契约：共用字段、上传信封、身份来源、计费币种、请求记录、工具字段、不采集清单 |
| [schema.md](schema.md) | 会话记录的字段与统计口径 |
| [sender/SPEC.md](sender/SPEC.md) | 发送端：采集、上报、身份与凭据、响应处理、配置 |
