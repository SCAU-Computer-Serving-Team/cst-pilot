...
// 「登录页 · 深色」AIR 背景专用色表。分布对齐浅色五色（等宽区间、色标落在 10/30/50/70/90%），
// 仅把底色从浅版的 #22265E 压深为 #161C4A，保持深色主题可读性；中间三色与浅色版一致。
const colors = ["#161C4A", "#3B4EA8", "#7785DE", "#BCC4F1", "#E4E9FA"];
...
writeFileSync(new URL("./blue-hour-air-dark-palette.png", import.meta.url), png);
console.log(`AIR dark palette: ${positions.map((position) => `${(position * 100).toFixed(1)}%`).join(", ")}`);
