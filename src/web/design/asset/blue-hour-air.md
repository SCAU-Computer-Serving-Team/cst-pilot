...
|---|---|
| 描边 | 顶部半透明白色，底部半透明蓝色 |
| 投影 | 使用 [DESIGN.md 的登录页面板两层投影](../../../../DESIGN.md#投影)，颜色与透明度只在该处定义 |
...
登录页沿用当前背景与表单，圆角、边框和投影只作用于外层面板。Shader 从画布高度 70% 至 93% 将色表坐标平滑过渡至冰蓝色标，保证灰色页脚可读；背景明暗由 Shader 控制，场景模糊只作用于面板背后的主页。
