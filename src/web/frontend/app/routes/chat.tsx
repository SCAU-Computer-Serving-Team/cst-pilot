...
import { Composer, sampleSessions, Sidebar } from "../app/shell";
  ...
  return (
    <div className="app-layout chat-layout">
      ...
      <main className="chat-main">
        <header className="chat-header"><HardDrive size={20} /><h1>{example?.title ?? "会话"}</h1>{example && <span className="preview-tag">设计预览 · 示例数据</span>}</header>
        {sessionId === "preview" && example ? <PreviewConversation /> : <div className="chat-empty">{example ? "此会话暂无对话示例。" : "会话接管将在下一阶段开放。"}</div>}
